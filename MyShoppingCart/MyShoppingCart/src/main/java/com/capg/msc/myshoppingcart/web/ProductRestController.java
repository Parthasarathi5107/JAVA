...
public class ProductRestController {
	...
	{
		if(product.getCategory().equals("Laptop") && product.getProductCost()>=10000)
			return service.saveProduct(product);
	
		else if(product.getCategory().equals("Camera") && product.getProductCost()>=15000)
			return service.saveProduct(product);
		else if(product.getCategory().equals("Chocolate") && product.getProductCost()>1)
			return service.saveProduct(product);
		else if(product.getProductCost() > 1)
			return service.saveProduct(product);
	...
	{
		if(category.equals("Laptop") || category.equals("Camera") || category.equals("Chocolate"))
		{
	...
	@GetMapping("/product/{category}/{range1}/{range2}")
	...
	@GetMapping("/product/rating/{category}/{rating}")
	public List<Product> getAllProductsByRating(@PathVariable String category,@PathVariable int rating)throws InvalidRatingException
	{
		if(rating > 5)
		{
		...
		return service.getAllProductsByCategoryAndRating(category,rating);
	}
